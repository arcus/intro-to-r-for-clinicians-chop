...
Ideally, we ask workshop TAs to be available for the duration of the workshop (`r print_time(workshop_date)` - `r print_time(workshop_date + dhours(5))`) to try to answer questions that come up in the chat, but if you're only available for part of that time, still reach out! In addition to the workshop itself, we'll have one 30-min meeting with just the instructors and TAs a few days before the workshop to touch base. That's it!
